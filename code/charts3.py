...
def totalAcquisition(threeOrgsFileSizeFixed, fiveOrgsFileSizeFixed, threeOrgsNumTubesFixed, fiveOrgsNumTubesFixed):
    ...
    #fig.layout.update(legend=dict(y=1.2, orientation="h"), width=1000)
    fig.layout.update(legend=dict(y=1.2, orientation="h"), width=1000, annotations=[
        dict(
            x=0.01, y=-0.3, showarrow=False, text="a) Varying number of tubes. File size is fixed", xref="paper", yref="paper", font=FONT
        ), dict(
            x=0.99, y=-0.3, showarrow=False, text="b) Varying file size. Number of tubes is fixed", xref="paper", yref="paper", font=FONT
        )
    ])
